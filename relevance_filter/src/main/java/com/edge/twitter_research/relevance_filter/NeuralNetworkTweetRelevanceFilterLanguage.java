...
public class NeuralNetworkTweetRelevanceFilterLanguage {

    public static void main(String[] args){

        if (args.length < 1){
            System.out.println("usage: NeuralNetworkTweetRelevanceFilterLanguage <root_path>");
            System.exit(-1);
        }

        String rootPath = args[0];
        File dataFolder = new File(rootPath + "/data/language");
        String resultFolderPath = rootPath + "/results/language/NeuralNetwork/";

        CrisisMailer crisisMailer = CrisisMailer.getCrisisMailer();
        Logger logger = Logger.getLogger(NeuralNetworkTweetRelevanceFilterLanguage.class);
