...
public class BayesNetTweetRelevanceFilterTweet {

    public static void main(String[] args){

        if (args.length < 1){
            System.out.println("usage: BayesNetTweetRelevanceFilterTweet <root_path>");
            System.exit(-1);
        }

        String rootPath = args[0];
        File dataFolder = new File(rootPath + "/data/tweet");
        String resultFolderPath = rootPath + "/results/tweet/BayesNet/";

        CrisisMailer crisisMailer = CrisisMailer.getCrisisMailer();
        Logger logger = Logger.getLogger(BayesNetTweetRelevanceFilterTweet.class);
